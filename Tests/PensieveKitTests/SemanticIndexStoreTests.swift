...
@Suite struct SemanticIndexStoreTests {
  ...
  @Test func upsertThenKNNReturnsNearestWithMetadata() async {
    ...
    let vectorA = await embedder.embed(["alpha"])![0]!
    let vectorB = await embedder.embed(["beta"])![0]!
  ...
  @Test func defaultScopeKNNExcludesArchived() async {
    let store = SemanticIndexStore(url: tempURL(), dimension: 8, embedderVersion: "stub:8")
    let embedder = StubEmbedder(dimension: 8)
    let vector = await embedder.embed(["x"])![0]!
  ...
  @Test func versionMismatchRebuildsEmpty() async {
    ...
    let vector = await embedder.embed(["x"])![0]!
    do {
      let indexStore = SemanticIndexStore(url: url, dimension: 8, embedderVersion: "stub:8")
      indexStore.upsert(row: .init(itemID: "a", kind: "node", nodeID: "n", state: "active",
                          contentHash: "h"), embedding: vector)
    }
  ...
  @Test func existingItemsReturnsHashMap() async {
    ...
    store.upsert(row: .init(itemID: "a", kind: "node", nodeID: "n", state: "active",
                            contentHash: "h1"), embedding: vector)
  ...
  @Test func knnExcludesArchivedByDefaultAndIncludesItWhenAsked() async throws {
    ...
    indexStore.upsert(row: .init(itemID: "a", kind: "node", nodeID: "n1",
                        state: NodeState.active.rawValue, contentHash: "h1"), embedding: activeVec)
    indexStore.upsert(row: .init(itemID: "b", kind: "node", nodeID: "n2",
