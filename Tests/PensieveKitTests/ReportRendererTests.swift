...
@Test func markdownIncludesRecommendationAndCaveats() {
  let cells = [CellScore(modelLabel: "apple/fm", isOnDevice: true, quality: 0.8, precision: nil, recall: nil, costUSD: 0,
                         latencyP50: 900, reproducedFabrication: false)]
  let rec = Recommendation(task: "narration", winner: "apple/fm", clearedBar: ["apple/fm"], reason: "incumbent clears")
  let scorecard = Scorecard(corpusHash: "abc123",
                            tasks: [TaskScorecard(task: "narration", cells: cells, recommendation: rec, judgeAgreement: 0.9)])
  let markdown = ReportRenderer.markdown(scorecard)
