...
@Test func v11AddsLabelColumnsDefaultingToEmpty() throws {
  ...
  let looseEnd = LooseEnd(nodeID: node.id, sourceEventID: event.id, text: "migrate auth later",
  ...
  let stored = try database.read { database in try LooseEnd.where { $0.id.eq(looseEnd.id) }.fetchOne(database) }
  #expect(stored?.label == "")
  #expect(stored?.labelSuggestion == "")

  // Values round-trip.
  try database.write { database in
    try LooseEnd.where { $0.id.eq(looseEnd.id) }
      .update { $0.label = LooseEndLabel.salient; $0.labelSuggestion = LooseEndLabel.noise }
      .execute(database)
  }
  let updated = try database.read { database in try LooseEnd.where { $0.id.eq(looseEnd.id) }.fetchOne(database) }
