...
private func seedLooseEnd(_ database: any DatabaseWriter, transcriptURL: URL,
                          citedIndex: Int, quote: String) throws -> LooseEnd {
  let (node, source) = try ProjectResolver(database: database).resolve(path: "/p/app", kind: SourceKind.claudeCode)
...
@Test func provenanceReturnsWindowAroundCitedUserPrompt() throws {
  let database = try openCanonicalDatabase(at: tempURL("prov-happy"))
...
@Test func provenanceClampsWindowAtEdges() throws {
  let database = try openCanonicalDatabase(at: tempURL("prov-edge"))
...
@Test func provenanceRejectsSamePhraseNonUserFalseMatch() throws {
  ...
  // a wrong highlight.
  let database = try openCanonicalDatabase(at: tempURL("prov-false"))
