...
- **Build/test bullet:** replace "Run the CLI: `swift run pensieve <subcommand>`…" with: the CLI is now an Xcode `tool` target embedded at `Pensieve.app/Contents/Helpers/pensieve`; build it via `xcodebuild -scheme pensieve build` (or the app scheme, which also embeds it) — **`swift run pensieve` no longer exists**; PensieveKit + tests stay SwiftPM (`./scripts/test.sh`).
