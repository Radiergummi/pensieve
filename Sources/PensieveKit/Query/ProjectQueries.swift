...
public enum ProjectQueries {
  public static func all(_ db: any DatabaseReader) throws -> [Node] {
    try db.read { db in try Node.order { $0.name }.fetchAll(db) }
  }

  public static func status(_ db: any DatabaseReader, name: String, limit: Int) throws -> ProjectStatus? {
  ...
  public static func status(_ db: any DatabaseReader, node: Node, limit: Int) throws -> ProjectStatus {
