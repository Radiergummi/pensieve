...
public struct NarrationCache: Sendable {
  ...
  private static func open(_ url: URL) -> (any DatabaseWriter)? {
    do {
      ...
      try pool.write { database in
        try database.execute(sql: "CREATE TABLE IF NOT EXISTS narration (key TEXT PRIMARY KEY, prose TEXT NOT NULL)")
      }
      return pool
    } catch {
      return nil
  ...
  public func get(_ key: String) -> String? {
    ...
    let result = try? database.read { database -> String? in
      try String.fetchOne(database, sql: "SELECT prose FROM narration WHERE key = ?", arguments: [key])
    }
    return result ?? nil
  }
  ...
  public func put(_ key: String, prose: String) {
    ...
    try? database.write { database in
      try database.execute(sql: "INSERT OR REPLACE INTO narration (key, prose) VALUES (?, ?)", arguments: [key, prose])
    }
  }
}
